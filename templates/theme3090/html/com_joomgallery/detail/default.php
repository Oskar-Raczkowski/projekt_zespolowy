...
$document->addScript('templates/'.$template->template.'/html/com_joomgallery/detail/js/jquery.validate.min.js');
$document->addScript('templates/'.$template->template.'/html/com_joomgallery/detail/js/additional-methods.min.js');
...
<?php     if(!$this->_user->get('id') && $this->_config->get('jg_namedanoncomment')): ?>
          <input required type="text" class="inputbox" name="cmtname" placeholder="<?php echo $this->_mainframe->getUserState('joom.comments.name', JText::_('COM_JOOMGALLERY_COMMON_GUEST')); ?>" tabindex="1" />
<?php     endif;
...
<?php     $rows = 4;
          if($this->_config->get('jg_smiliesupport')):
            ...
            <textarea required cols="40" rows="<?php echo $rows; ?>" name="cmttext" class="inputbox" tabindex="100" ><?php echo $this->_mainframe->getUserState('joom.comments.text'); ?></textarea>
